...
CREATE TABLE public.users (
  ...
  country_code TEXT, phone TEXT, country TEXT,
...
CREATE TABLE public.email_otps (
  id BIGSERIAL PRIMARY KEY, user_id BIGINT, email TEXT NOT NULL,
  code_hash TEXT NOT NULL,
  purpose TEXT NOT NULL CHECK (purpose IN ('signup','reset')),
  expires_at TIMESTAMPTZ NOT NULL,
  used BOOLEAN NOT NULL DEFAULT false, attempts INT NOT NULL DEFAULT 0,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE public.orders (
  ...
  plan TEXT, balance TEXT, price_usd NUMERIC(10,2),
  network TEXT, tx_hash TEXT, hash_file TEXT,
...
CREATE TABLE public.deposits (
  id BIGSERIAL PRIMARY KEY, user_id BIGINT NOT NULL, order_id BIGINT,
  amount_usdt NUMERIC(12,2), network TEXT, tx_hash TEXT,
...
CREATE TABLE public.withdrawals (
  id BIGSERIAL PRIMARY KEY, user_id BIGINT NOT NULL,
  amount_usdt NUMERIC(12,2), wallet_address TEXT, network TEXT,
  status TEXT DEFAULT 'pending',
  requested_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  processed_at TIMESTAMPTZ, admin_notes TEXT
);
...
CREATE TABLE public.refunds (
  id BIGSERIAL PRIMARY KEY, user_id BIGINT NOT NULL, order_id BIGINT,
  amount_usdt NUMERIC(12,2), reason TEXT,
  status TEXT DEFAULT 'pending',
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  processed_at TIMESTAMPTZ, admin_id TEXT
);
...
CREATE TABLE public.complaints (
  ...
  severity TEXT DEFAULT 'normal', status TEXT DEFAULT 'open',
  assigned_to TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  resolved_at TIMESTAMPTZ
);
...
CREATE TABLE public.kyc_records (
  id BIGSERIAL PRIMARY KEY, user_id BIGINT,
  doc_type TEXT, doc_number TEXT, full_name TEXT,
  country TEXT, document_image TEXT,
  status TEXT DEFAULT 'pending',
...
CREATE TABLE public.messages (
  id BIGSERIAL PRIMARY KEY, case_type TEXT, case_id BIGINT,
  user_id BIGINT, direction TEXT, body TEXT, sender_name TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE public.email_queue (
  id BIGSERIAL PRIMARY KEY, to_email TEXT, to_name TEXT,
  subject TEXT, body TEXT, template TEXT,
...
CREATE TABLE public.internal_notes (
  id BIGSERIAL PRIMARY KEY, case_type TEXT, case_id BIGINT,
  author TEXT, body TEXT, visibility TEXT DEFAULT 'internal',
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE public.escalations (
  id BIGSERIAL PRIMARY KEY, case_type TEXT, case_id BIGINT,
  from_level TEXT, to_level TEXT, reason TEXT,
...
CREATE TABLE public.financial_reports (
  id BIGSERIAL PRIMARY KEY, report_kind TEXT, period TEXT,
  metric TEXT, amount_usdt NUMERIC(14,2), count INT,
...
CREATE TABLE public.activity_logs (
  id BIGSERIAL PRIMARY KEY, actor_id TEXT, actor_type TEXT,
  action TEXT, target_type TEXT, target_id BIGINT,
  metadata JSONB, page_url TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE public.trade_accounts (
  ...
  password_hash TEXT NOT NULL, password_plain TEXT,
  plan TEXT NOT NULL,
...
CREATE TABLE public.trade_positions (
  id BIGSERIAL PRIMARY KEY, trade_account_id BIGINT NOT NULL,
  pair TEXT NOT NULL, side TEXT NOT NULL,
  lots NUMERIC(8,2) NOT NULL, leverage INT NOT NULL DEFAULT 100,
  open_price NUMERIC(18,8) NOT NULL, close_price NUMERIC(18,8),
...
CREATE TABLE public.trade_sessions (
  token TEXT PRIMARY KEY, trade_account_id BIGINT NOT NULL,
  expires_at TIMESTAMPTZ NOT NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- Seed admin + manager users (both flagged is_admin for admin panel access)
INSERT INTO public.users (name, email, password_hash, email_verified, is_admin) VALUES
  ('Admin', 'admin@mail', 'pbkdf2$100000$6cuorRxkGE5Dx8M/+uFCbw==$/DHkSk2wR06qqqxaD8mwuDpa8ch5b+rd1xV716ZJla0=', true, true),
  ('Manager', 'manager@mail', 'pbkdf2$100000$9N4LxuGoK6XBJ/hfM62uyg==$7ndsBAGBUIJzmyt+IBnK2Zg1aJCUTnMiZCClT80FEpg=', true, true);
